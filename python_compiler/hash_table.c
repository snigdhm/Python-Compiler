...
typedef struct hash_elem_t {
	...
	char name[20];	// Pointer to the stored element
	char type[20];	
	int data_type;
	int return_type;
	int offset;
...
char* ht_put(hashtable_t* hasht, char* key, char name[20], char type[20],int data_type,int return_type,int offset,int num_param, int scope)
{
	//if(data == NULL)
		//return NULL;
	...
	while(e != NULL)
	{
		if(!strcmp(e->key, key))
		{
			...
			if(scope!=0 && e->offset==0)
				e->offset=offset;

			if(strcmp(e->type,type))
			{e->data_type=data_type;strcpy(e->type,type);}
			return e->name;
		}
		e = e->next;
	}
...
int ht_get(hashtable_t* hasht, char* key)
{
	unsigned int h = ht_calc_hash(key) % hasht->capacity;
	hash_elem_t* e = hasht->table[h];
	while(e != NULL)
...
int ht_getparam(hashtable_t* hasht, char* key,char *type)
{
	unsigned int h = ht_calc_hash(key) % hasht->capacity;
	hash_elem_t* e = hasht->table[h];
	while(e != NULL)
...
int ht_gettype(hashtable_t* hasht, char*key, char* type)
{
	unsigned int h = ht_calc_hash(key) % hasht->capacity;
	hash_elem_t* e = hasht->table[h];
	while(e != NULL)
...
int ht_get_ret_type(hashtable_t* hasht, char*key, char* type)
{
	unsigned int h = ht_calc_hash(key) % hasht->capacity;
	hash_elem_t* e = hasht->table[h];
	while(e != NULL)
...
void ht_list_values(FILE *outputFile,hashtable_t* hasht,int j)
{
